...
namespace shield
{
  ...
  void BsplineOptimizer::optimize()
  {
    ...
    for (int i = 0; i < point_num_; ++i)
      for (int j = 0; j < dim_; ++j)
      {
        ...
        if (dim_ != 1)
          cij = max(min(cij, bmax[j % 3] - 1e-3), bmin[j % 3] + 1e-3);
        q[dim_ * i + j] = cij;
      }
      ...
      const std::vector<double> &x, std::vector<double> &grad, double &f_combine)
  {
    ...
    if (cost_function_ & SMOOTHNESS)
    {
      ...
      if (print_cost)
        std::cout << "smoothness cost: " << f_smoothness << std::endl;
      for (int i = 0; i < point_num_; i++)
        for (int j = 0; j < dim_; j++)
    ...
    if (cost_function_ & DISTANCE)
    {
      ...
      if (print_cost)
        std::cout << "distance cost: " << f_distance << std::endl;
      for (int i = 0; i < point_num_; i++)
        for (int j = 0; j < dim_; j++)
    ...
    if (cost_function_ & FEASIBILITY)
    {
      ...
      if (print_cost)
        std::cout << "feasibility cost: " << f_feasibility << std::endl;
      for (int i = 0; i < point_num_; i++)
        for (int j = 0; j < dim_; j++)
    ...
    if (cost_function_ & START)
    {
      ...
      if (print_cost)
        std::cout << "start cost: " << f_start << std::endl;
      for (int i = 0; i < 3; i++)
        for (int j = 0; j < dim_; j++)
    ...
    if (cost_function_ & END)
    {
      ...
      if (print_cost)
        std::cout << "end cost: " << f_end << std::endl;
    ...
    if (cost_function_ & GUIDE)
    {
      ...
      if (print_cost)
        std::cout << "guide cost: " << f_guide << std::endl;
      for (int i = 0; i < point_num_; i++)
        for (int j = 0; j < dim_; j++)
    ...
    if (cost_function_ & WAYPOINTS)
    {
      ...
      if (print_cost)
        std::cout << "waypoints cost: " << f_waypoints << std::endl;
      for (int i = 0; i < point_num_; i++)
        for (int j = 0; j < dim_; j++)
    ...
    if (cost_function_ & VIEWCONS)
    {
      ...
      if (print_cost)
        std::cout << "view cost: " << f_view << std::endl;
      for (int i = 0; i < point_num_; i++)
        for (int j = 0; j < dim_; j++)
    ...
    if (cost_function_ & MINTIME)
    {
      ...
      if (print_cost)
        std::cout << "time cost: " << f_time << std::endl;
    ...
    if (cost_function_ & SWARM)
    {
      ...
      if (print_cost)
        std::cout << "swarm cost: " << f_swarm << std::endl;
      for (int i = 0; i < point_num_; i++)
        for (int j = 0; j < dim_; j++)
          grad[dim_ * i + j] += ld_swarm_ * g_swarm_[i](j);
    }
