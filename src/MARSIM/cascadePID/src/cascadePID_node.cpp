...
{
    ROS_WARN("pose command received");
    pose_cmd = msg;
    control_flag = 1;
}
...
//   ROS_WARN("fuel_position_cmd_callback yaw command received: %f", cmd->yaw/M_PI*180.0);
...
void run_control(const ros::TimerEvent& event)
{
        ...
        if(control_flag == 1)
        {
            ...
            // ROS_WARN("cmd yaw = %f",yaw_des/M_PI*180.0);
            t2 = ros::Time::now();
        ...
        // ROS_WARN("cmd yaw 2 = %f",yaw_des/M_PI*180.0);
