...
namespace shield
{
  ...
  int Astar::search(const Eigen::Vector3d &start_pt, const Eigen::Vector3d &end_pt, bool optimistic)
  {
    ...
    bool inflate_check = {edt_env_->sdf_map_->getInflateOccupancy(start_pt) == 1};

    double max_search_time_temp = max_search_time_;
    if((end_pt - start_pt).norm() > resolution_*100)
    {
      max_search_time_temp = max_search_time_ * 5.0;
    }
    if((end_pt - start_pt).norm() > resolution_*200)
    {
      max_search_time_temp = max_search_time_ * 10.0;
    }
    ...
    while (!open_set_.empty())
    {
      ...
      /*********** LZC added end point safety check *************/

      /**********************************************************/
      if (reach_end && checkLine(cur_node->position, end_pt, 2))
      {
      ...
      if (((ros::Time::now() - t1).toSec() > max_search_time_temp))
      {
      ...
      for (double dx = -resolution_; dx <= resolution_ + 1e-3; dx += resolution_)
        for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
          for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_)
          {
            ...
            if ((edt_env_->sdf_map_->getOccupancy(nbr_pos) == SDFMap::OCCUPIED) ||
                (!inflate_check && edt_env_->sdf_map_->getInflateOccupancy(nbr_pos) == 1) ||
            ...
            for (double l = 0.1; l <= len + 1e-2; l += 0.1)
            {
              ...
              if ((edt_env_->sdf_map_->getOccupancy(ckpt) == SDFMap::OCCUPIED) ||
                  (!inflate_check && edt_env_->sdf_map_->getInflateOccupancy(ckpt) == 1) ||
  ...
  std::vector<Eigen::Vector3d> Astar::
      getPath()
  {
    return path_nodes_;
  }
  ...
  /*********************** LZC Added **********************/
  bool Astar::checkLine(const Eigen::Vector3d &cur_pt, const Eigen::Vector3d &next_pt, int type)
  {
    Vector3d dir = next_pt - cur_pt;
    double len = dir.norm();
    dir.normalize();
    for (double l = 0.1; l <= len + 1e-2; l += 0.1)
    {
      Vector3d ckpt = cur_pt + l * dir;
      if (type == 1)
      {
        if (edt_env_->sdf_map_->getOccupancy(ckpt) == SDFMap::OCCUPIED)
        {
  ...
  bool Astar::checkPath(const vector<Eigen::Vector3d> &path)
  {
    if (path.empty())
    {
      return false;
    ...
    for (int i = 0; i < path.size() - 1; i++)
    {
      if (!checkLine(path[i], path[i + 1], 2))
      {
        return false;
      }
    }
  ...
  bool Astar::updateWaypoint(const vector<Eigen::Vector3d> &path, const Eigen::Vector3d &cur_pt, Eigen::Vector3d &next_pt, int interval)
  {
    double distance = 0;
    ...
    if (interval + index >= path.size())
      interval = path.size() - index - 1;

    while (interval >= 0)
    {
      if (!checkLine(cur_pt, path[index + interval], 2))
      {
        interval--;
      }
      else
      {
        next_pt = path[index + interval];
        return true;
      }
    }
    return false;
